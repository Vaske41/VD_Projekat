$(document).ready(function(){
    ...
    artists.forEach(artist => {
        ...
        <div class="col-xl-5 col-lg-4 col-md-2 text-center w-75">
            ...
            <p>${artist.bioSRB}</p>
            <p><button class="btn btn-success" onclick="generatePDF(${artist.id})">Pregled dela</button></p>
        </div>
...
function generatePDF(id){
  props.fileName = artists[id - 1].forename + " " + artists[id - 1].surname + " - Pregled umetnina";
  ...
  for(let i=0;i<artwork.length;i++){
    table.push([i+1, artwork[i].name, artwork[i].price, artwork[i].age, artwork[i].typeSRB]);
  }
